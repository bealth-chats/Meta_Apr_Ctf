...
# Actually, the reason it crashes on allocate(2) might be because
# our overwritten `fd` pointer is unaligned or invalid tcache pointer,
# OR `malloc` checks the size of the target block!
# In tcache, it doesn't check size. Wait, glibc 2.34 DOES check size and alignment!
# In glibc 2.34:
# 1. Tcache ptr must be 16-byte aligned. `free_got` (0x35f8) is NOT 16-byte aligned. It ends in 8!
# Let's verify: 0x35f8 % 16 = 8.
